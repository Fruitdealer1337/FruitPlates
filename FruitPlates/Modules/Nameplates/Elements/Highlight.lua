...
local function PickHighlightIcon(frame)
    if frame.ClassIcon and RegionVisible(frame.ClassIcon) and IconLooksAttached(frame.Health, frame.ClassIcon) then
        return frame.ClassIcon
    end

    if frame.RaidIcon and RegionVisible(frame.RaidIcon) and IconLooksAttached(frame.Health, frame.RaidIcon) then
...
function NP:Update_Highlight(frame)
    ...
    if frame.Health and not frame.Health:IsShown() then
        HideHighlightFrame(hl)
        return
    end
    ...
    elseif mode == "ICON" then
        ...
        shown = ShowGlow(hl.HealthGlow, frame.Health, frame, size, alpha, color)
        if icon then
            ShowGlow(hl.IconGlow, icon, frame, size, alpha, color)
        else
            HideGlow(hl.IconGlow)
        end
    end
