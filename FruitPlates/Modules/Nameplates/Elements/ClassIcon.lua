...
local CLASS_ICON_CIRCLE_TEXTURES = {
    WARRIOR = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\WARRIOR.tga]],
    PALADIN = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\PALADIN.tga]],
    HUNTER = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\HUNTER.tga]],
    ROGUE = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\ROGUE.tga]],
    PRIEST = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\PRIEST.tga]],
    DEATHKNIGHT = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\DEATHKNIGHT.tga]],
    SHAMAN = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\SHAMAN.tga]],
    MAGE = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\MAGE.tga]],
    WARLOCK = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\WARLOCK.tga]],
    DRUID = [[Interface\AddOns\FruitPlates\Media\Textures\ClassIconsCircle\DRUID.tga]],
}
...
local function GetClassIconStyle(classDB, family)
    if not classDB then return "SQUARE" end
    if family ~= "enemy" and family ~= "friendly" then return "SQUARE" end
    if classDB[family .. "Style"] == "CIRCLE" then return "CIRCLE" end
    return "SQUARE"
end

local function GetClassIconTexture(class, style)
    if style == "CIRCLE" and CLASS_ICON_CIRCLE_TEXTURES[class] then
        return CLASS_ICON_CIRCLE_TEXTURES[class]
    end
...
local function SetClassIconFrameBorder(icon, shown)
    if not icon then return end
    local method = shown and "Show" or "Hide"
    if icon.backdrop then icon.backdrop[method](icon.backdrop) end
    if icon.bordertop then icon.bordertop[method](icon.bordertop) end
    if icon.borderbottom then icon.borderbottom[method](icon.borderbottom) end
    if icon.borderleft then icon.borderleft[method](icon.borderleft) end
    if icon.borderright then icon.borderright[method](icon.borderright) end
end
...
function NP:Configure_ClassIcon(frame)
    ...
    local style = GetClassIconStyle(classDB, family)
    if style == "CIRCLE" then
        HideClassIconOutline(icon)
        SetClassIconFrameBorder(icon, false)
    else
        SetClassIconFrameBorder(icon, true)
...
function NP:Update_ClassIcon(frame)
    ...
    local family = PlateIconFamily(frame.UnitType)
    local style = GetClassIconStyle(classDB, family)
    local texture = GetClassIconTexture(frame.UnitClass, style)
    if not texture then
        self:ClearClassIcon(frame)
        return
    end
    ...
    if icon._fruitplatesClass ~= frame.UnitClass
        or icon._fruitplatesStyle ~= style
